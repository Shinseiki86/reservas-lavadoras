...
class Controller extends BaseController
{
	...
	public function __construct($requireAuth=true)
	{
        ...
        if(auth()->check() and is_null(session()->get('menusLeft')))
            \LAVA\Http\Controllers\App\MenuController::refreshMenu();
	...
	protected function buttonDeleteOper($model, $modelDescrip)
	{
		if(\Entrust::hasRole(['admin', 'superoper', 'cooroper']))
			return \Form::button('<i class="fas fa-trash fa-fw" aria-hidden="true"></i>',[
				'class'=>'btn btn-xs btn-danger btn-delete',
				'data-toggle'=>'modal',
				'data-id'=> $model->getKey(),
				'data-modelo'=> str_upperspace(class_basename($model)),
	...
	{
		return class_exists($class) ? $class : '\\LAVA\\Models\\'.basename($class);
	}
